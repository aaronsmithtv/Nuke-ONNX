 ...
 * Contains functions and structures for tensor manipulation independent of
 * Nuke/NDK
 */
...
public:
   ...
   */
  static void findMinMax(const std::vector<float> &tensorData, float &minValue,
                         float &maxValue) {
    if (tensorData.empty()) {
      minValue = 0.0f;
      maxValue = 1.0f;
   ...
   */
  static void findMinMaxMultiChannel(const std::vector<float> &tensorData,
    ...
    if (minValue == std::numeric_limits<float>::max() ||
        maxValue == std::numeric_limits<float>::lowest() ||
        std::isnan(minValue) || std::isinf(minValue) || std::isnan(maxValue) ||
        std::isinf(maxValue)) {
      minValue = 0.0f;
      maxValue = 1.0f;
    }
  ...
  static float normalize(float value, float min, float max) {
    ...
    // Handle edge case
    if (min >= max || std::isnan(min) || std::isinf(min) || std::isnan(max) ||
        std::isinf(max)) {
      return 0.5f;
    }
   ...
   */
  static float getTensorValue(const std::vector<float> &tensorData, int x,
                              int y, int channelIdx, int width, int height,
                              bool isSingleChannel, bool doNormalize,
                              float minValue, float maxValue) {
    // Validate parameters
