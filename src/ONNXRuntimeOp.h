 ...
 * Allows loading and execution of ML models on images with GPU acceleration
 * support. Supports multiple inputs (1-10) for complex ONNX models.
 */
...
public:
  ...
  void _request(int x, int y, int r, int t, DD::Image::ChannelMask channels,
                int count) override;
  void engine(int y, int x, int r, DD::Image::ChannelMask channels,
              DD::Image::Row &row) override;
...
private:
  ...
  // Processing state
  std::unique_ptr<ONNXModelManager>
      _modelManager; // Manages ONNX model and inference
  std::unique_ptr<ONNXInferenceProcessor>
      _inferenceProcessor;           // Handles inference workflow
  ...
  void preprocessImage(const DD::Image::Iop *input,
                       std::vector<float> &inputTensor);

  // Output handling
  void findMinMaxValues(); // Find min/max values for normalization
  void setupOutputChannels(
      DD::Image::ChannelSet &channels); // Configure output channels
