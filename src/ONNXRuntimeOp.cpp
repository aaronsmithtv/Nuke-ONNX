...
ONNXRuntimeOp::ONNXRuntimeOp(Node* node) : Iop(node),
    _modelPath(""),
    _useGPU(false),
    _normalize(false),
    ...
    if (for_real) {
        ...
        if (!_modelManager->isLoaded() && strcmp(_modelPath, "") != 0) {
            try {
                ...
                // loadModel might still throw if path is invalid before manager->load is called
                error("%s", e.what()); 
            }
...
void ONNXRuntimeOp::_request(int x, int y, int r, int t, ChannelMask channels, int count)
{
...
void ONNXRuntimeOp::engine(int y, int x, int r, ChannelMask channels, Row& row)
{
    ...
    { // Scope for lock guard
        ...
        if (!_cacheValid) {
            try {
                ...
                if (_normalize) { // No need to check _processingDone, exception handles failure
                    findMinMaxValues();
                }
            ...
            _processingDone = processing_succeeded; // Update status based on try-catch
    ...
    if (!_processingDone || y < 0 || y >= _outputHeight) { // Check _processingDone status set by try-catch
        if (input(0)) {
            input0().get(y, x, r, channels, row);
        } else {
            row.erase(channels);
...
{
    if (!_modelManager->isLoaded()) {
        throw ConfigurationException("Attempted to process image but no model is loaded");
    }
    ...
    if (_imgWidth <= 0 || _imgHeight <= 0) {
        throw ConfigurationException("Invalid input dimensions from Nuke format: " + 
                                   std::to_string(_imgWidth) + "x" + std::to_string(_imgHeight));
    }
    ...
    for (int i = 0; i < _activeInputs; i++) {
        ...
        if (currentInput == nullptr) {
            if (i == 0) { // Should not happen due to earlier check, but defensive
                throw ConfigurationException("Primary input (input 0) became disconnected unexpectedly");
            }
            // Mark corresponding tensor as invalid if an optional input is disconnected
    ...
    if (_processedData.empty()) {
        ...
        throw InferenceException("Inference completed but resulted in empty output data");
    }
    
    _inferenceProcessor->getOutputDimensions(_outputWidth, _outputHeight, _outputChannelCount);
    _isSingleChannel = _inferenceProcessor->isSingleChannelOutput();
}

void ONNXRuntimeOp::preprocessImage(const Iop* input, std::vector<float>& inputTensor)
{
    if (!input) {
        throw InvalidArgumentException("Null input pointer passed to preprocessImage");
    }
    ...
    try {
        ...
        throw PreprocessException(std::string("Error during image preprocessing: ") + e.what());
    }
}
...
void ONNXRuntimeOp::findMinMaxValues()
{
    ...
    if (_isSingleChannel) {
        ...
        TensorProcessor::findMinMaxMultiChannel(
            _processedData, _minValue, _maxValue,
            _outputChannelCount, _outputWidth, _outputHeight
        );
    }
}
...
void ONNXRuntimeOp::loadModel()
{
    ...
    try {
        _modelManager->load(_modelPath, _useGPU); // Throws ModelLoadException on failure
        ...
        int channels;
        if (_modelManager->getOutputDimensions(_outputWidth, _outputHeight, channels)) {
            _outputChannelCount = channels;
            _isSingleChannel = (channels == 1);
        } else {
            warning("Could not retrieve fixed output dimensions from model. Output size might adapt to input.");
        ...
        error("Failed to load model: %s", e.what()); // Use error() for user feedback
        throw; // Re-throw to signal failure to caller (e.g., knob_changed)
    } catch (const std::exception& e) {
        error("Unexpected error loading model: %s", e.what());
        throw ModelLoadException(std::string("Unexpected: ") + e.what()); // Wrap and re-throw
    }
}
...
void ONNXRuntimeOp::updateDimensions()
{
    ...
    try {
        ...
        // Debug output
        // printf("ONNX Resizing: %dx%d -> %dx%d\n", _imgWidth, _imgHeight, _outputWidth, _outputHeight);
...
void ONNXRuntimeOp::displayModelInfo()
{
    ...
    std::string infoStr = Utils::buildModelInfoString(
        ...
        _outputHeight,
        _activeInputs,
        _modelManager->getInputCount(),
        _modelManager->getInputNames(),
        [this](int idx) { return input(idx) != nullptr; },
        _normalize,
        _minValue,
        _maxValue,
        &DD::Image::getName
    ...
    // Display the message using the simplified utility function (prints to stderr)
...
const char* ONNXRuntimeOp::Class() const
{
    return CLASS;
}

const char* ONNXRuntimeOp::node_help() const
{
    return HELP;
}
...
const Iop::Description ONNXRuntimeOp::description(CLASS, "Image/ONNX Runtime", build);