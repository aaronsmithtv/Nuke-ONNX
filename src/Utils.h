 ...
 */
inline DD::Image::Tile extractTile(const DD::Image::Iop& input, DD::Image::ChannelSet channels) {
...
inline DD::Image::Tile extractTile(const DD::Image::Iop* input, DD::Image::ChannelSet channels) {
    if (!input) {
        throw InvalidArgumentException("Null input pointer provided to extractTile");
    }
    return extractTile(*input, channels);
}

/**
 * Convert a tile to NCHW tensor format (batch=1) maintaining original image dimensions
 * Format: [1, channels, height, width]
 */
inline void tileToNCHWTensor(
    const DD::Image::Tile& tile,
    std::vector<float>& tensor,
    int width, int height, int channels
) {
    if (width <= 0 || height <= 0 || channels <= 0) {
        throw PreprocessException("Invalid dimensions for tensor conversion: " + 
                                  std::to_string(width) + "x" + std::to_string(height) + " C:" + std::to_string(channels));
    }
    ...
    for (int c = 0; c < channels; c++) {
        ...
        switch (c) {
            case 0: chan = DD::Image::Chan_Red; break;
            case 1: chan = DD::Image::Chan_Green; break;
            case 2: chan = DD::Image::Chan_Blue; break;
            case 3: chan = DD::Image::Chan_Alpha; break;
            default: continue;  // Skip unsupported channels
        }
        ...
        if (!(tile.channels() & DD::Image::ChannelSet(chan))) {
            ...
            std::fill(tensor.begin() + offset, tensor.begin() + offset + height * width, 0.0f);
            continue;
        }
 ...
 * Returns: 0 for red/x, 1 for green/y, 2 for blue/z, 3 for alpha/w, -1 for unknown
 */
inline int getChannelComponentIndex(DD::Image::Channel ch) {
    ...
    if (component == "red" || component == "r" || component == "x") return 0;
    if (component == "green" || component == "g" || component == "y") return 1;
    if (component == "blue" || component == "b" || component == "z") return 2;
    if (component == "alpha" || component == "a" || component == "w") return 3;
    
    return -1;
}

/**
 * Process a Nuke row from tensor data based on the format (single/multi-channel)
 */
inline void processTensorDataToRow(
    const std::vector<float>& tensorData,
    int y, int x, int r,
    DD::Image::ChannelMask channels,
    DD::Image::Row& row,
    const DD::Image::Row& inputRow,
    ...
    foreach (z, channels) {
        ...
        bool isCustomChannel = (z != DD::Image::Chan_Red && 
                               z != DD::Image::Chan_Green && 
                               z != DD::Image::Chan_Blue && 
                               z != DD::Image::Chan_Alpha);
        ...
        if (isCustomChannel) {
            ...
            if (componentIndex >= 0 && componentIndex < channelCount) {
                ...
                for (int i = x; i < endX; i++) {
                    outPtr[i] = TensorProcessor::getTensorValue(
                        tensorData, i, y, channelToUse,
                        outputWidth, outputHeight,
                        isSingleChannel, normalize,
                        minValue, maxValue
                    );
                }
            } else {
            ...
            int outChannel = -1;
            if (z == DD::Image::Chan_Red && channelCount > 0) outChannel = 0;
            else if (z == DD::Image::Chan_Green && channelCount > 1) outChannel = 1;
            else if (z == DD::Image::Chan_Blue && channelCount > 2) outChannel = 2;
            else if (z == DD::Image::Chan_Alpha && channelCount > 3) outChannel = 3;

            if (outChannel >= 0 && outChannel < channelCount) {
                for (int i = x; i < endX; i++) {
                    outPtr[i] = TensorProcessor::getTensorValue(
                        tensorData, i, y, outChannel,
                        outputWidth, outputHeight,
                        isSingleChannel, normalize,
                        minValue, maxValue
                    );
...
inline std::string buildModelInfoString(
    const std::string& modelInfoString,
    bool useGPU,
    bool isSingleChannel,
    ...
    const std::function<bool(int)>& inputConnectionStatus,
    bool normalize,
    float minValue,
    float maxValue,
    ...
    additionalInfo << "Processing mode: " << (isSingleChannel ? "Single channel" : "Multi-channel") << "\n";
    ...
    additionalInfo << "\nActive Inputs: " << activeInputs << " of " << modelInputCount << " required\n";
    ...
    if (normalize) {
        additionalInfo << "Normalization: Enabled (min=" << minValue << ", max=" << maxValue << ")\n";
    } else {
